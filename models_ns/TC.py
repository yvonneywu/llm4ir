...
class TC(nn.Module):
    ...
    def forward(self, features_aug1, features_aug2):
        ...
        t_samples = torch.randint(seq_len - self.timestep, size=(1,)).long().to(self.device)  # generate a random interger that will be used as
        #time stamps

        nce = 0  # average over timestep and batch
        encode_samples = torch.empty((self.timestep, batch, self.num_channels)).float().to(self.device) #[timestep, batch, final_output]
        ...
        ### if we want to use same transformer.. our model ends here...
